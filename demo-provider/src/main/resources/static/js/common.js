 ...
 * 重置制定的字段
 */
function resetFields(vue,name,fieldNames){
    //this.fields.forEach(function(e){e.resetField()});
    vue.$refs[name].fields.forEach(function (e) {
        if (fieldNames.indexOf(e.prop.indexOf()) > -1) {
            e.resetField()
        }
    })
};
 ...
 * @returns {*}
 */
function transCaseStat(value) {

    var result = '未知';

    switch (value) {
        case '0':result = '未调度'; break;
        case '1':result= '外勤退回'; break;
        case '2':result= '调度退回'; break;
        case '3':result= '已调度'; break;
        case '4':result= '已删除'; break;
        case '5':result= '已接受'; break;
        case '6': result = '已注销'; break;
        case '7': result = '已完成'; break;
        case '8': result = '客户撤案'; break;
        case '9': result = '转单案件'; break;
        case '10': result = '已转单'; break;
        case '11': result = '客户撤案'; break;
        case '12': result = '转咨询'; break;
        default: result = '未知'; break;
    }
    return result;
};

function transCaseType(value) {

    var result = '未知';

    switch (value) {
        case 1:result= 'IT报案'; break;
        case 2:result= '工程报案'; break;
        case 9: result = '其他报案'; break;
        default: result = '未知'; break;
    }
    return result;
};
 ...
 * @returns {*}
 */
function transMyCaseStat(value) {
...
function transferBillImg(cellvalue, options, rowObject ) {
    if (rowObject["ncIsTransfer"] == 1) {
    	return '<img width="20px" height="20px" src="../statics/img/zhuandan5.png">'+cellvalue;
    }
    return "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+cellvalue;
};

function transVipImg(cellvalue, options, rowObject ) {
    if (rowObject["cuIsVip"] == 1) {
    	return '<img width="20px" height="20px" src="../statics/img/vip.png">'+cellvalue;
    }
    return "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+cellvalue;
};
 ...
 * @returns {*}
 */
function transSmsisread(smIsRead) {
    if (smIsRead == 0) {
        return '否';
    }else if (smIsRead == 1) {
        return '是';
...
function getWorkTime(){
    ...
    Ajax.request({
        url: "../sys/date/getworktime",
        async: false,
        successCallback: function (r) {
            console.log(r.time);
            strDt = (r.time+":00").replace(/-/g,"/");
            time = new Date(strDt);//将字符串转化为时间 
            console.log(time);
        }
    });
    return time;
